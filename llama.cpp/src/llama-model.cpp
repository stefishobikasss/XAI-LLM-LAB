    ...
    llm_build_llama(const llama_model & model, const llm_graph_params & params, ggml_cgraph * gf) : llm_graph_context(params) {
        ...
        for (int il = 0; il < n_layer; ++il) {
            ...
            if (model.layers[il].ffn_gate_inp == nullptr) {
                ...
                cb(cur, "ffn_out", il);
                char tensor_name[64];
                snprintf(tensor_name, sizeof(tensor_name), "ffn_out_%d", il);
                ggml_tensor *cur = ggml_get_tensor(ctx0, tensor_name);
                if (cur && cur->data) {
                    printf("🔥 Dumping layer %d activations to activations_layer%d.csv\n", il, il);

                    char filename[64];
                    snprintf(filename, sizeof(filename), "activations_layer%d.csv", il);
                    FILE *f = fopen(filename, "w");
                    if (!f) {
                        perror("🔥 ERROR opening file");
                    } else {
                        int n_neurons = cur->ne[0];
                        int n_tokens = cur->ne[1];
                        float *data = (float *)cur->data;

                        printf("🔥 Tensor shape: [%d, %d] (neurons, tokens)\n", n_neurons, n_tokens);

                        for (int t = 0; t < n_tokens; t++) {
                            fprintf(f, "token_pos=%d,", t);  // or get from params.pos if available
                            for (int i = 0; i < n_neurons; ++i) {
                                fprintf(f, "%.4f", data[i + t * n_neurons]);
                                if (i < n_neurons - 1) fprintf(f, ",");
                            }
                            fprintf(f, "\n");
                        }
                ...
                // if (cur && cur->data) {
                //     char filename[128];
                //     snprintf(filename, sizeof(filename), "/tmp/activations_layer%d.csv", il);

                //     char cwd[1024];
                //     if (getcwd(cwd, sizeof(cwd)) != NULL) {
                //         fprintf(stderr, "🔥 Hook triggered for layer %d\n", il);
                ...
                //         fprintf(stderr, "🔥 Tensor shape: [%d neurons, %d tokens]\n", n_neurons, n_tokens);

                //         for (int t = 0; t < n_tokens; t++) {
                //             int pos = t;
                //             fprintf(f, "token_pos=%d,", pos);
                //             for (int i = 0; i < n_neurons; ++i) {
                //                 fprintf(f, "%.4f", data[i + t * n_neurons]);
                //                 if (i < n_neurons - 1) fprintf(f, ",");
                //             }
                ...
                //     }
                // } else {
                //     fprintf(stderr, "🔥 cur is null or cur->data is null, skipping CSV dump\n");
                // }
                // // //if (params.n_tokens == 1 || params.n_tokens == 2) {  // optional limit to small batches
                // // char cwd[1024];
                ...
                // // char filename[62];
                // // snprintf(filename, sizeof(filename), "activations_layer%d.csv", il);
                // // printf("🔥 Dumping layer %d activations to %s\n", il, filename);
                // // FILE *f = fopen(filename, "w");
                // // if (!f) {
                // //     perror("🔥 ERROR opening file");
                // // } else if (cur && cur->data) {
                //     int n_neurons = cur->ne[0];  // usually dimension of FFN hidden layer
                //     int n_tokens = cur->ne[1];   // number of tokens in the batch
                ...
                //     fclose(f);
                // } else {
                //     printf("🔥 Skipped writing CSV: cur=%p, data=%p\n", (void*)cur, cur ? cur->data : nullptr);
                // }
