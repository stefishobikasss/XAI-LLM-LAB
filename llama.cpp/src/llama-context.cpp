...
#include <sys/stat.h>  // for stat (to check if file exists / is empty)
...
// static const char* my_llama_token_to_str(const llama_vocab & vocab, llama_token token_id) {
//     if (token_id >= vocab.n_tokens()) return "[UNK]";
//     return vocab.byte_to_token.at(token_id).c_str();
// }
...
int llama_context::decode(const llama_batch & batch_inp) {
    ...
    do {
        ...
        // === BEGIN HOOK: Dump token + logits ===
    {
        const ggml_tensor * t = t_logits;
        const int dim = t->ne[0];  // vocab size
        const int n_tok = n_outputs;

        // FILE* f = fopen("logits_activations.csv", "w");
        ...
        if (f) {
            ...
            for (int i = 0; i < n_tok; i++) {
                llama_token token_id = ubatch.token[i];

                // Token string to buffer
                char tok_buf[128];
                llama_token_to_piece(&model.vocab, token_id, tok_buf, sizeof(tok_buf), 1, false);
                fprintf(f, "\"%s\"", tok_buf);

                const float* row = logits + (n_outputs_prev + i) * dim;
                for (int j = 0; j < dim; j++) {
                    fprintf(f, ",%.6f", row[j]);
                }
                fprintf(f, "\n");
...
// === END HOOK ===
        ...
        // LOG LOGITS TO CSV
//         // === BEGIN HOOK: Dump token-wise activations ===
//         FILE* f = fopen("logits_activations.csv", "w");

//         fprintf(f, "token");
//         int dim = cur->ne[0]; // hidden size / logits dimension
//         for (int h = 0; h < dim; h++) {
//             fprintf(f, ",act%d", h);
//         }
//         fprintf(f, "\n");
...
//             const llama_token token_id = batch.token[i];
//             const char* tok_str = llama_token_to_str(model->vocab, token_id);
//             fprintf(f, "\"%s\"", tok_str);

//             for (int j = 0; j < dim; j++) {
//                 float val = ((float*)cur->data)[i * dim + j]; // get activation
//                 fprintf(f, ",%.6f", val);
//             }
//             fprintf(f, "\n");
//         }
...
// // === END HOOK ===
        ...
        // Pseudocode — place this inside your activation logging loop
        // FILE* f = fopen("logits_activations.csv", "w");
        ...
        //     const char* tok_str = llama_token_to_str(ctx, tokens[i]); // get token text

        //     fprintf(f, "\"%s\"", tok_str); // write token as string

        //     for (int j = 0; j < hidden_size; j++) {
        //         fprintf(f, ",%.6f", activations[i][j]); // write each hidden unit
        //     }
        //     fprintf(f, "\n");
        // }
        ...
        // if (n_outputs > 0) {
        //     std::ofstream file("logits_activations.csv", std::ios::app);
        //     if (file.is_open()) {
        //         for (int i = 0; i < n_outputs; ++i) {
        //             for (int j = 0; j < n_vocab; ++j) {
        //                 file << logits[(n_outputs_prev + i) * n_vocab + j];
        //                 if (j < n_vocab - 1) file << ",";
        //             }
        //             file << "\n";
        //         }
