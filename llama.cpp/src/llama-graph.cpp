...
ggml_tensor * llm_graph_context::build_moe_ffn(
    ...
    ggml_set_name(moe_out, tensor_name);
...
ggml_tensor * llm_graph_context::build_inp_embd(ggml_tensor * tok_embd) const {
    ...
    //     ctx->cb_func = [](const llama_ubatch & ubatch, ggml_tensor * t, const char * name, int il) {
    //     std::string filename = "dump_" + std::string(name) + "_layer" + std::to_string(il) + ".csv";
    ...
    //             fprintf(file, "%f", data[i * n0 + j]);
    //             if (j < n0 - 1) fprintf(file, ",");
    //         }
    //         fprintf(file, "\n");
    //     }
